...
function downloadName(storagePath: string) {
  const name = storagePath.split("/").pop() || "duewise-file";
  return name.replace(/["\\\r\n]/g, "-");
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { storagePath } = downloadUrlSchema.parse(await request.json());
    ...
    const [url] = await adminStorage()
      ...
      .getSignedUrl({
        ...
        responseDisposition: `attachment; filename="${downloadName(storagePath)}"`
      });
