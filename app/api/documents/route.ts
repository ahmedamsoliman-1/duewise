...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getOrSet(
      `documents:${user.uid}`,
      async () => {
        const snapshot = await userCollection(user.uid, 'documents').orderBy('createdAt', 'asc').get();
...
export async function POST(request: NextRequest) {
  try {
    const user = await requireUser(request);
    const payload = clean(documentSchema.parse(await request.json()));
    
    const ref = await userCollection(user.uid, 'documents').add({
      ...payload,
    ...
    // Invalidate cache
    await invalidate(`documents:${user.uid}`);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const payload = clean(documentSchema.partial().parse(body));
    delete payload.id;
    
    const ref = userCollection(user.uid, 'documents').doc(id);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await userCollection(user.uid, 'documents').doc(id).delete();
